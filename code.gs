...
function handleCalendarEvent(e) {
  try {
    ...
    // I am going to choose participant A as the active user and participant B as my secondary email for testing purposes. The participant B will be chosen from a guest list just in case we need more emails.
    ...
    // To make it simple I just chose the first index of my guest list as my participantB
    var participantB = guestList[0];
    var participantBEmail = participantB.getEmail();
    var participantBName = "participantB";

    // Now creating the folder with the given naming convention and I will use the driveapp library to create the folder.
    var folderName = participantAName + " " + participantBName + " Handoff";
    var folder = DriveApp.createFolder(folderName);

    // Sharing the folder with participant A and saving the handoff details.
    ...
    PropertiesService.getScriptProperties().setProperty("handoff_" + eventId, JSON.stringify(handoffData));
    // I am logging just to see if it executed
    Logger.log("Folder created for the event: " + eventId);

    // Creating a pre-populated form link to match the event id and emailing to the participant B.
    var prepopulatedLink = FORM_URL + "?" + EVENT_ID_KEY + "=" + encodeURIComponent(eventId);
    ...
    Logger.log("Pre-populated form link emailed to Participant B: " + participantBEmail);
...
// I have created a Dummy function below for testing the above function. For production purposes we have a simulate a proper calendar event using the above function using a timer trigger.
  ...
  var dummyEvent = {
    calendarEvent: {
      ...
      getGuestList: function () {
        return [{
          getEmail: function () { return "My secondary email"; }, // I used my secondary email for testing (gmail.com)
...
// Onsubmit function to get details from form. Just to let you that this apps script project is binded to my test form.
function onFormSubmit(e) {
  try {
    ...
    itemResponses.forEach(function (itemResponse) {
      ...
      // Here we are looking for the event id to match with the event id given in the testHandleCalendarEvent
    ...
    var handoffDataString = PropertiesService.getScriptProperties().getProperty("handoff_" + eventId);
    ...
    var pdfTitle = "Handoff Document between Participant A and Participant B";
    ...
    var message = "Hello participants ,\n\nThe handoff process has been completed. The folder is saved in Participant A's Drive Folder.\n\nRegards.";
